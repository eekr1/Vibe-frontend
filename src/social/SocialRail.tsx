...
export function SocialRail({ mode, onBadgeChange, onClose, onNavigate, open }: Props) {
  ...
  async function refresh() {
    ...
    setError(null);
    try {
      const [friendData, requestData, presenceData, summaryData] = await Promise.all([
        listFriends(),
        listFriendRequests(),
        listFriendPresence(),
        getNotificationSummary()
    ...
    <aside aria-hidden={!open} aria-label="Social Rail" className={`social-rail ${open ? "is-open" : ""} is-${mode}`} id="social-rail">
      <div className="social-rail-panel" role="complementary">
        ...
        <div className="social-rail-summary" aria-live="polite">
          ...
          <span><strong>{Math.max(summary.actionableCount, incoming.length)}</strong> requests</span>
        </div>
            ...
            <div className="social-rail-list">
              {filteredFriends.map((profile) => (
                <article className="social-rail-friend" key={profile.id}>
                  <SocialIdentity context={describePresence(presence[profile.id])} initialRelationship={initialRelationship("friends")} onChanged={() => void refresh()} onNavigate={navigate} profile={profile} />
                </article>
              ))}
            </div>
